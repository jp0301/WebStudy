...
// 다양한 방법들 중 한가지(그나마 가장 기본적이고 쉬운 것을 골라...)이다.
// 학습을 마친 이후에...
...
public class MyUtil
{
	...
	//   → 『pageCount = dataCount / numPerPage;』
	//   그런데 이 때, 나머지 2 개의 게시물을 출력해주기 위해서는
	//   페이지가 하나 더 필요하다.
	//   → 『pageCount++;』
	...
	public String getIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		*/
		...
		if(listUrl.indexOf("?") != -1) // 링크를 설정할 URL 에 『?』가 들어있으면...
			listUrl = listUrl + "&";
		else						   // 링크를 설정할 URL 에 『?』가 들어있으면...
			listUrl = listUrl + "?";
		//-- 예를 들어,
		//   검색값이 존재하면 이미 request 값에
		//   searchKey와 searchValue가 들어있는 상황이므로
		//   『&』를 붙여서 추가해 주어야 한다.
